...
# Description: updateMasterKey.sh.
# Author: MindX SDK
# Create: 2021
# History: NA

CUR_PATH=$(cd "$(dirname "$0")" || { warn "Failed to check path/to/updateMasterKey.sh" ; exit ; } ; pwd)
...
if [ ! -O "$0" ]; then
    record_info "Error: updateMasterKey.sh is not belong to the current user. Failed to update masterkey."
    exit 1
fi

if [ $(id -u) -eq 0 ]; then
    record_info "Warning: updateMasterKey.sh is executed by root."
fi
...
if [ ! -O "../../../set_env.sh" ]; then
    record_info "Error: set_env.sh is not belong to the current user. Failed to update masterkey."
    exit 1
fi
...
if [ -d "./keys" ]; then
    if [ -O "./keys" ]; then
        ...
        record_info "Error: The destination directory \"keys\" is not belong to the current user. Failed to update masterkey."
        exit 1
    fi
fi
...
func() {
    echo "Usage:"
    echo "./updateMasterKey.sh -d <number>"
    echo "Description:"
    echo "-d update advance time in days."
    exit 1
}
...
do
    if [ "$args" -gt "$argc" ]; then
      record_info "Error: Wrong usage, too many arguments. Failed to update masterkey."
      func
      exit 1
    fi
    case "$arg" in
        ...
        ;;
        ?)
        record_info "Error: Wrong usage, unknown argument. Failed to update masterkey."
        func
        exit 1
        ;;
    esac
...
if [ -n "$days" ]; then
    if [[ ! "$days" =~ ^[0-9]*$ ]]; then
        record_info "Error: invalid param of -d, required int. Failed to update masterkey."
        exit 1
    fi
else
    record_info "Error: Required argument \"-d <int>\" is missing. Failed to update masterkey."
    func
    exit 1
fi
...
if [ -L "$log_file" ] || [ ! -f "$log_file" ] || [ ! -O "$log_file" ]; then
    record_info "Error: cannot find the logging.conf. Failed to update masterkey."
    exit 1
fi
max_size=$((1024 * 10))
file_size=$(ls -l "$log_file" | awk '{ print $5 }')
if [ "$file_size" -gt "$max_size" ]; then
    record_info "Error: invalid logging.conf, the file size is too large. Failed to update masterkey."
    exit 1
fi
levelc=$(cat ../../../config/logging.conf | grep console_level= | wc -l)
if [ "$levelc" -ne 1 ]; then
    record_info "Error: console_level key should be exact one in config file. Failed to update masterkey."
    exit 1
fi
...
if ! [[ "${old_level}" =~ ${pattern} ]]; then
    record_info "Error: console_level in logging.conf is not set or invalid. Failed to update masterkey."
    exit 1
fi
sed -i "s/"${old_level}"/console_level=0/" ../../../config/logging.conf
record_info "Change the console_level to 0 in updateMasterKey."
# run
../../../bin/UpdateMasterKey "$days"
sed -i "s/console_level=0/"${old_level}"/" ../../../config/logging.conf
record_info "Change the console_level to its original level in updateMasterKey."
record_info "updateMasterKey execute finished, see result in sdk log."
exit 0