...
void turn_on_robot::Control()
{
  ...
  {
    RCLCPP_WARN(this->get_logger(),"Serial port exception: %s", e.what()); 
    std::this_thread::sleep_for(std::chrono::milliseconds(100)); // 如果串口异常，等待100ms后继续
  }
  catch (const std::exception & e)
  {
    RCLCPP_WARN(this->get_logger(),"Exception in control loop: %s", e.what()); 
    std::this_thread::sleep_for(std::chrono::milliseconds(100)); // 通用异常，等待100ms后继续
  }
}
}
/**************************************
